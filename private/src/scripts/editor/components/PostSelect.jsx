...
class PostSelect extends Component {
  ...
  fetchSelectedSidebar = () => {
    ...
    return wp.apiRequest({ path: `/wp/v2/sidebar/${this.props.value}` }).then((item) => {
      this.setState(
        {
          selected: {
  ...
  fetchSidebars() {
    wp.apiRequest({ path: '/wp/v2/sidebar' }).then(this.handleApiResult);
  }
  ...
  fetchSidebarPage = () => {
    ...
    wp.apiRequest({ path: `/wp/v2/sidebar/?page=${currentPage}` }).then(this.handleApiResult);
  };
