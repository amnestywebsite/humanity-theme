...
const DisplayFeed = ({ overrideTypes, style, prefix, showAuthor, showPostDate, amount }) => {
  ...
  const fetchPostsByPostType = () => {
    ...
    wp.apiRequest({
      ...
      setResults(alterResults(res)); // Alter results and update state
      setLoading(false); // Reset loading state
    });
  };

  // Fetch posts when component mounts
  useEffect(fetchPostsByPostType, [overrideTypes]);
  ...
  if (isGrid) {
    const className = `grid ${[1, 2, 3, 5, 6, 7].includes(amount) ? `grid-${amount}` : 'grid-many'}`;
    return (
