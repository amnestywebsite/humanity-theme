...
const DisplayCustom = ({ custom = [], setAttributes, style, prefix, showAuthor, showPostDate }) => {
  ...
  const createUpdateMediaAttribute =
    (index) =>
      ...
      if (custom[index]) {
        ...
        setAttributes({
          custom: custom.map((item, i) =>
            index === i ? { ...item, featured_image, featured_image_id } : item,
          ),
        });
      } else {
        // Add new item with default values
        setAttributes({
          custom: [...custom, { ...defaultObject, featured_image, featured_image_id }],
        });
      }
    };
  ...
  const createUpdateAttribute = (index) => (key) => (value) => {
    if (custom[index]) {
      // Update existing item
      setAttributes({
        custom: custom.map((item, i) => (index === i ? { ...item, [key]: value } : item)),
      });
    } else {
      // Add new item with default values
      setAttributes({
        custom: [...custom, { ...defaultObject, [key]: value }],
      });
    }
  };
  ...
  const itemsToRender = custom.length ? custom : [{ ...defaultObject }];
  ...
  const gridClass = [1, 2, 3, 5, 6, 7].includes(itemsToRender.length)
    ? `grid-${itemsToRender.length}`
    : 'grid-many';
