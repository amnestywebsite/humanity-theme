...
const Control = ({ attributes, className, isOpen, onClick }) => {
      ...
      <fieldset className="checkboxGroup-list">
        {Object.keys(attributes.options).map((key) => (
          <span className="checkboxGroup-item" key={key}>
            <input id={makeId(key)} className="screen-reader-text" type="radio" name={attributes.name} checked={key === attributes.active} value={key} readOnly={true} />
