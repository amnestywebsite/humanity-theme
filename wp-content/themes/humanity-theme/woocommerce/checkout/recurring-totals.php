...
<tr class="recurring-totals">
	<th colspan="3"><?php /* translators: [front] Donate */ esc_html_e( 'Recurring totals', 'woocommerce-subscriptions' ); ?></th>
</tr>
...
<?php if ( 0 === absint( $recurring_cart->next_payment_date ) ) continue // phpcs:ignore ?>
	...
	<?php if ( $display_th ) : $display_th = false; // phpcs:ignore ?>
		<th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>" colspan="2"><?php /* translators: [front] Donate */ esc_html_e( 'Subtotal', 'woocommerce-subscriptions' ); ?></th>
		<td data-title="<?php /* translators: [front] Donate */ esc_attr_e( 'Subtotal', 'woocommerce-subscriptions' ); ?>"><?php wcs_cart_totals_subtotal_html( $recurring_cart ); ?></td>
	<?php else : ?>
...
<?php foreach ( WC()->cart->get_coupons() as $code => $coupon ) : ?>
	...
	<?php if ( 0 === absint( $recurring_cart->next_payment_date ) ) continue // phpcs:ignore ?>
		<?php foreach ( $recurring_cart->get_coupons() as $recurring_code => $recurring_coupon ) : ?>
			<?php if ( $recurring_code !== $code ) continue; // phpcs:ignore ?>

			<tr class="cart-discount coupon-<?php echo esc_attr( $code ); ?> recurring-total">
			<?php if ( $display_th ) : $display_th = false; // phpcs:ignore ?>
				<th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>" colspan="2"><?php wc_cart_totals_coupon_label( $coupon ); ?></th>
				<td data-title="<?php wc_cart_totals_coupon_label( $coupon ); ?>"><?php wcs_cart_totals_coupon_html( $recurring_coupon, $recurring_cart ); ?>
				<?php wcs_cart_coupon_remove_link_html( $recurring_coupon ); ?></td>
			<?php else : ?>
		...
		<?php foreach ( WC()->cart->get_taxes() as $tax_id => $tax_total ) : ?>
			<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
				...
				<?php foreach ( $recurring_cart->get_tax_totals() as $recurring_code => $recurring_tax ) : ?>
					<?php if ( ! isset( $recurring_tax->tax_rate_id ) || $recurring_tax->tax_rate_id !== $tax_id ) continue; // phpcs:ignore ?>
...
<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
	...
	<?php if ( $display_th ) : $display_th = false; // phpcs:ignore ?>
		<th rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>" colspan="2"><?php /* translators: [front] Donate */ esc_html_e( 'Recurring total', 'woocommerce-subscriptions' ); ?></th>
		<td data-title="<?php /* translators: [front] Donate */ esc_attr_e( 'Recurring total', 'woocommerce-subscriptions' ); ?>"><?php wcs_cart_totals_order_total_html( $recurring_cart ); ?></td>
	<?php else : ?>
